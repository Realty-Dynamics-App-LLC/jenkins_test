# Check if the index idx_geom exists before creating it
        index_exists_query = """
        SELECT 1 
        FROM pg_indexes 
        WHERE tablename = 'fl_geojson' 
        AND indexname = 'idx_geom';
        """
        result = connection.execute(index_exists_query).fetchone()
        
        if not result:
            connection.execute("CREATE INDEX idx_geom ON fl_geojson USING SPGIST(wkb_geometry);")
            print("CREATED idx_geom index")
        else:
            print("Index idx_geom already exists, skipping creation")-- psql -U postgres -d postgres -h 20.84.118.206 -p 5432 -f final.geojson.sql
